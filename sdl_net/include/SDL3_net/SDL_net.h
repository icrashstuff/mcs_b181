...
/**
 * Get the remote port of a stream socket. (This is a modification by mcs_b181)
 *
 * This reports the port of the remote side of a stream socket, which might
 * still be pending connnection.
 *
 * \param sock the stream socket to query.
 * \returns the socket's remote port, or 0 on error; call SDL_GetError()
 *          for details.
 *
 * \threadsafety It is safe to call this function from any thread.
 *
 * \since This function is available since mcs_b181
 */
extern SDL_DECLSPEC Uint16 SDLCALL SDLNet_GetStreamSocketPort(SDLNet_StreamSocket *sock);
