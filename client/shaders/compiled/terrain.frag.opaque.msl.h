...
fragment main0_out main0(main0_in in [[stage_in]], constant ubo_frag_t& ubo_frag [[buffer(0)]], constant ubo_lightmap_t& ubo_lightmap [[buffer(1)]], texture2d<float> tex_atlas [[texture(0)]], sampler tex_atlasSmplr [[sampler(0)]])
{
    ...
    float4 _235 = out.out_color;
    float3 _243 = mix(_235.xyz, ubo_frag.fog_color.xyz, float3(smoothstep(ubo_frag.fog_min, ubo_frag.fog_max, frag.fog_dist)));
    out.out_color.x = _243.x;
    out.out_color.y = _243.y;
    out.out_color.z = _243.z;
    return out;
}
