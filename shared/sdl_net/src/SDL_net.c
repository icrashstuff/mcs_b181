...
SDLNet_Server *SDLNet_CreateServer(SDLNet_Address *addr, Uint16 port)
{
    ...
    int resuse = 1;
    setsockopt(server->handle, SOL_SOCKET, SO_REUSEADDR, (const char *) &resuse, sizeof (resuse));  // if this fails, oh well.
...
Uint16 SDLNet_GetStreamSocketPort(SDLNet_StreamSocket *sock)
{
    if (!sock) {
        SDL_InvalidParamError("sock");
        return 0;
